...
with the web UI rebuilt 1:1 against the `../SR_Dashboard/` design prototype.
It **coexists** with the legacy Python/Next stack in the parent repo
(`../backend`, `../frontend`) — v2 lives entirely under `v2/`.
...
- **GHCR publishing**: the additive `build-and-push-v2` job in
  `.github/workflows/build-and-push.yml` (repo root) publishes
  `ghcr.io/magma-devs/smart-router-dashboard/{api,web}` (tags: `VERSION`,
  `major.minor`, `latest` on main; other branches get a `-<branch>` suffix).
  The v1 backend/frontend images are untouched. `make build` reproduces the
  same images locally on an isolated buildx builder (`srdash-builder`) — on a
  Docker daemon shared with sibling projects, BuildKit's shared cache can
