...
async function customerAdmin(email = "dana@customer.example") {
...
describe("who gets marked", () => {
  ...
  it("marks nothing on-prem, where the first admin is the customer's own", async () => {
    ...
    expect((await firstRun("admin@customer.example")).isMagmaAccount).toBe(false);
  });
  ...
  it("never marks an invited account, even one invited on managed", async () => {
    ...
    const [row] = await t.db.select().from(users).where(eq(users.email, "dana@customer.example"));
...
describe("the member list shows it", () => {
  it("labels the row in the JSON every role can read", async () => {
    ...
    expect(rows.map((r) => r.email).sort()).toEqual(["dana@customer.example", magma.email]);
    expect(rows.find((r) => r.email === magma.email)?.isMagmaAccount).toBe(true);
    expect(rows.find((r) => r.email === "dana@customer.example")?.isMagmaAccount).toBe(false);
  });
  ...
  it("carries it into the CSV, which is the artifact an auditor reads", async () => {
    ...
    const customerRow = rows.find((r) => r.includes("dana@customer.example"));
