...
export function GET() {
  return NextResponse.json({
    apiUrl:
      process.env.DASHBOARD_API_URL ?? process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000",
    localMode: (process.env.DASHBOARD_LOCAL_MODE ?? process.env.NEXT_PUBLIC_LOCAL_MODE) === "true",
