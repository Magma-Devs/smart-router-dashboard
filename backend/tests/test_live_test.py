...
class TestLiveTestEndpoint:
    ...
    def test_live_test_endpoint_validation(self):
        ...
        response = client.post("/api/live-test/", json=request_data, headers=_basic_header("admin", "password"))
...
class TestCrossValidationEndpoint:
    ...
    def test_cross_validation_endpoint_validation(self):
        ...
        response = client.post("/api/live-test/cross-validation/", json=request_data, headers=_basic_header("admin", "password"))
        assert response.status_code == 422  # Validation error
    ...
    def test_cross_validation_endpoint_quorum_logic_validation(self):
        ...
        assert "quorum_min cannot be greater than quorum_max" in response.json()["detail"]
    ...
    def test_cross_validation_endpoint_successful_request(self, mock_client_class):
        ...
        mock_response.headers = {"content-type": "application/json", "lava-provider-address": "provider1"}
        ...
        assert headers["X-Host"] == "test-chain.jsonrpc.lavapro.xyz"
    ...
    def test_cross_validation_endpoint_rest_interface(self, mock_client_class):
        ...
        assert call_args[1]["url"] == "https://lavapro.xyz:8443/block/latest"
    ...
    def test_cross_validation_endpoint_failed_request(self, mock_client_class):
        ...
        assert response.status_code == 200  # Endpoint returns 200 even for failed requests
        response_data = response.json()
