...
class LiveTestRequest(BaseModel):
    ...
    interface: str = Field(..., description="Interface type (jsonrpc, rest, tendermintrpc, grpc)")
    interface_command: str = Field(..., description="Interface command/path for the request")
    number_of_requests: int = Field(default=1, ge=1, le=200, description="Number of requests to send (1-200)")
    skip_cache: bool = Field(default=False, description="Skip cache by adding lava-force-cache-refresh header")
...
class LiveTestResponse(BaseModel):
    ...
    latency_stats: Dict[str, float] = Field(..., description="Latency statistics (min, max, avg, p50, p90)")
    cached_count: int = Field(..., description="Number of responses served from cache")
    non_cached_count: int = Field(..., description="Number of responses not served from cache")
    responses: List[Dict[str, Any]] = Field(default_factory=list, description="Sample responses")
...
async def run_live_test(
    ...
    try:
        ...
        latency_stats = {
            ...
            "p50": statistics.quantiles(latencies, n=2)[0] if len(latencies) >= 2 else statistics.mean(latencies) if latencies else 0,
            "p90": statistics.quantiles(latencies, n=10)[8] if len(latencies) >= 10 else statistics.mean(latencies) if latencies else 0,
        }
        ...
        for r in responses:
            ...
            key = next((k for k in headers.keys() if k.lower() == "lava-provider-address"), None)
...
async def cross_validation_endpoint(
    ...
    if request.quorum_min > request.quorum_max:
        raise HTTPException(
            status_code=400,
            detail="quorum_min cannot be greater than quorum_max"
        )
