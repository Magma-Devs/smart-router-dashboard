...
| `POST /api/trace/:guid/explain` | — | `TraceExplainResult` — Claude's account of that relay: `explanation` (`summary` / `timeline` / `findings` / **`notDetermined`**), `model`, `linesConsidered`. **Separate route because it spends money** — explaining on page load would bill once per reader of a link whose purpose is being pasted, and leave no way to ask again. **No log parser**: the router's zerolog JSON goes to the model as written; only the ANSWER is structured. 404 `ai_disabled` without `TRACE_AI_ENABLED`+key, 422 `no_lines`, 502 `explain_failed` (the lines are unaffected). Tighter `TRACE_AI_RATE_LIMIT_MAX`. See [`docs/trace-ai-page/RELAY-TRACE.md`](docs/trace-ai-page/RELAY-TRACE.md) |
