...
| Endpoint URLs | `localhost:<port>` comes from SR_CONFIG's listen ports, gateway hostnames from helm values' `publicUrls` — a mount never has both. Anything that renders or dials an endpoint address must resolve public → local → **nothing** (`epHttpUrl` / `epWsUrl` in `components/endpoints/bits.tsx`), never hardcode `localhost`. "Nothing" renders as the shared `NO_ADDRESS_SHORT` / `NO_ADDRESS_NOTE` copy — a bare `—` reads as a bug or a loading state when it is really a fact about the mount. |
| Cloud-only columns | Values with no self-hosted equivalent at all (JWT suffix, "last used", "created") are **removed**, not rendered as `—`. A dash means "no data yet"; these can never have data, and a row of dashes is what got the Endpoints page filed as a bug (MAG-2537). Compute-unit quota / RPS cap / regions stay pinned `null` because a panel *labels* them "not tracked". |
