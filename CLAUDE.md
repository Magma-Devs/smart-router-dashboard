...
| `GET /api/trace/:guid` | — | `RelayTrace` — one relay's log trail from Loki. **Makes NO model call**: `lines` (raw, oldest first — sorted on the NANOSECOND, since a relay writes most of its trail inside one or two ms), `searched`, `truncated`, `aiAvailable`, `model`. The GUID is validated (`^[0-9]{1,20}$`, ≤ 2⁶⁴−1), never escaped, because the LogQL is built by concatenation. Window widens 15m→168h, stopping at the first hit. 400 on a non-GUID; **503 `log_store_not_configured`** when `LOKI_URL` is unset and **503 `log_store_unavailable`** when Loki is down — neither is ever an empty trace. Under the global `RATE_LIMIT_MAX`: it costs nothing |
| `POST /api/trace/:guid/explain` | body: `{provider?, model?, apiKey?}` | `TraceExplainResult` — the model's account of that relay: `explanation` (`summary` / `timeline` / `findings` / **`notDetermined`**), `model`, `provider`, `usedCallerKey`, `linesConsidered`. **Bring-your-own-key**: a caller may supply its own provider/model/key, which is used for that ONE call and never persisted, logged or echoed back — so each person spends their own budget and the deployment holds no secret everyone who can reach it could spend. With no body, the deployment's own config answers. **Separate route because it spends money** — explaining on page load would bill once per reader of a link whose purpose is being pasted. **No log parser**: the router's zerolog JSON goes to the model as written; only the ANSWER is structured. 404 `ai_disabled` when neither the caller nor the deployment has a key, 422 `no_lines`, 502 `explain_failed` (the lines are unaffected). Tighter `TRACE_AI_RATE_LIMIT_MAX`. See [`docs/trace-ai-page/RELAY-TRACE.md`](docs/trace-ai-page/RELAY-TRACE.md) |
| `POST /api/trace/models` | body: `{provider?, apiKey?}` | `TraceModelsResponse` — `{provider, models[], defaultModel}`, asked of the PROVIDER rather than hardcoded, because a checked-in list is wrong the week a model ships or retires (`gemini-2.5-flash` closed to new users mid-build). Same bring-your-own-key rule as the explain route; an omitted provider means "whatever this deployment is configured for", which is what the settings picker opens on. Gemini's list is filtered to `generateContent` support minus tts/image/embedding. 502 `model_list_failed` |
