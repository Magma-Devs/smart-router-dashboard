...
| `POST /auth/invite/accept` | — | Redeem: `{ token, password }` → `{ user }`, or `{ token, googleIdToken }` → `{ user, sessionId }`. The account is created with the **invited** address. Only the Google path opens a session — it holds a one-shot id_token and has no second sign-in to fall back on; the password path lets the credentials sign-in do it. 403 on an address mismatch, 410 on a dead link |
