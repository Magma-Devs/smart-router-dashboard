...
| `POST /auth/password/forgot` | — | `{ email }` → always `202` (managed), in the same time for a member and a stranger — the link is issued after the reply. `404` on-prem — there is nowhere to send it. Public |
...
| `POST /api/account/password` | — | `{ current, next }` — signs out your other devices, keeps this one. Sign-in's rate limit (10/min): it tests a credential |
