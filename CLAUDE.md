...
| `GET /api/metrics/providers` | `window`, `spec?` | `{ providers: ProviderMetrics[] }` — roster with requests, uptime, p95, **errorRate**, selection scores, health, latestBlock, **blockLag**, **role**, **apiInterface**, inFlight. **One row per (upstream × chain)** — an upstream serving two chains gets a row on each, with its own numbers and its own **`role`** (`primary`/`backup` from *either* config format — helm `is_backup` or SR_CONFIG's `backup-direct-rpc` section — matched to the config node name case-insensitively; null only when the config doesn't describe that upstream on that chain) |
...
| Upstream `role` | **Both** formats mark backups — helm `is_backup`, SR_CONFIG's `backup-direct-rpc` section. Role is resolved per (upstream × chain), because the same upstream is routinely primary on one chain and backup on another. It is null only when the mounted config doesn't describe that upstream on that chain. |
| Config name ↔ metric label | The join key between the mounted config and Prometheus is the upstream's **name**: the router labels every series with `provider.Name` from *its own* config (`endpoint_id` / `provider_address`), and a helm deployment renders that from the node name here — the case need not survive. Match it **case-insensitively**; a missed join is silent, and reads as a real zero rather than as no data. |
