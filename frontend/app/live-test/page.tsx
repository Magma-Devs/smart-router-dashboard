  ...
  useEffect(() => {
    const fetchChains = async () => {
      ...
      try {
        ...
        const availableChainConfigs = chains.filter(chain => configuredChains.includes(chain.value))
        ...
        <div className="grid gap-6">
          <Card className="border-2 border-primary/20 bg-gradient-to-br from-card/50 to-card shadow-lg hover:shadow-xl transition-all duration-200">
            <CardHeader className="border-b border-border/20 bg-gradient-to-r from-primary/10 to-background pb-4">
              <CardTitle className="text-xl font-bold bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">Configuration</CardTitle>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {configuredInterfaces.map((iface) => {
                          ...
                          const displayName = iface === "jsonrpc" ? "JSON-RPC" :
                                            iface === "tendermintrpc" ? "TendermintRPC" :
                                            iface === "rest" ? "REST" :
                                            iface === "grpc" ? "gRPC" : iface
                          return (
                            <Button
                              key={iface}
                              ...
                              className={cn(
                                ...
                                (iface === "jsonrpc" ? "bg-blue-500 hover:bg-blue-600" :
                                 iface === "tendermintrpc" ? "bg-green-500 hover:bg-green-600" :
                                 iface === "rest" ? "bg-purple-500 hover:bg-purple-600" :
                                 iface === "grpc" ? "bg-orange-500 hover:bg-orange-600" : ""),
                                "hover:opacity-90"
                              )}
